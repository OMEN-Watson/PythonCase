...
def count_dict_difference(A, B):
    ...
    for Akey in A.keys():
        ...
        if Akey in B:
            timesBkey=B[Akey]
            if(timesAkey-timesBkey>0):
                resultDic[Akey]=timesAkey-timesBkey
        else:
            resultDic[Akey]=timesAkey
    print(resultDic)
    return resultDic
...
def test_count_dict_difference():
    ...
    assert count_dict_difference({'d': 3, 'e': 1, 'z': 1, 's': 1, 'i': 1, 'r': 1, 'a': 2, 'n': 1, 't': 1},
     {'e': 2, 'x': 1, 'g': 1, 's': 1, 'p': 1, 'i': 1, 't': 1, 'a': 2, 'n': 1, 'r': 1}) == {'z': 1, 'd': 3}
